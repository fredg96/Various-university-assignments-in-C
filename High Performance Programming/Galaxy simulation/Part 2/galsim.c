...
typedef struct tree_node
{
    ...
    int isLeaf;
} __attribute((packed)) node_t;

void fileRead(char * file, data_t **stored_data, int N);
void fileWrite(char * file, data_t **stored_data, int N);
...
void getAcceleration(node_t **node_trav, data_t *curr_object_i, vector *acc, double theta_max, double G);
...
int main(int argc, char *argv[]){
    ...
    data_t *stored_data = (data_t *)malloc(sizeof(data_t)*N); 
    fileRead(input_file, &stored_data, N);
    node_t *root = NULL;
    ...
    for(k=0; k<nsteps; k++){
        ...
        for(i=0;i<N; i++){
            acc.x = 0;
            acc.y = 0;
            getAcceleration(&root, &stored_data[i], &acc, theta_max, G);
            stored_data[i].vel_x += delta_t*acc.x;
            stored_data[i].vel_y += delta_t*acc.y;

        }
        for(i=0;i<N;i++){
            stored_data[i].pos_x += delta_t*stored_data[i].vel_x;
            stored_data[i].pos_y += delta_t*stored_data[i].vel_y;
    ...
    fileWrite("./result.gal", &stored_data, N);
    free(stored_data);
    return 0;
}

void fileRead(char * file, data_t **stored_data_ptr, int N){ 
...
void fileWrite(char * file, data_t **stored_data_ptr, int N){ 
...
void getAcceleration(node_t **node_trav, data_t *curr_object_i, vector *acc, double theta_max, double G){
    ...
    if((*node_trav) == NULL){
        ...
        if(theta<=theta_max && (*node_trav)->totalMass > 0){ 
            ...
            getAcceleration(&((*node_trav)->topLeftTree), curr_object_i, acc, theta_max, G);
            getAcceleration(&((*node_trav)->topRightTree), curr_object_i, acc, theta_max, G);
            getAcceleration(&((*node_trav)->botLeftTree), curr_object_i, acc, theta_max, G);
            getAcceleration(&((*node_trav)->botRightTree), curr_object_i, acc, theta_max, G);
        }
